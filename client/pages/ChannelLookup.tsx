            ...
            <div className="space-y-6">
              <div className="flex items-center gap-5">
                <div className="w-[80px] h-[80px] rounded-[14px] overflow-hidden bg-[rgba(58,42,238,0.15)] flex items-center justify-center">
                  {chatData.avatar_url ? (
                    <img
                      src={getAvatar()}
                      alt={chatData.display_name || "Channel"}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <span className="text-[#3A2AEE] text-[32px] font-bold">
                      {(chatData.display_name || 'C').charAt(0).toUpperCase()}
                    </span>
                  )}
                </div>
                <div>
                  <h1 className="font-sans font-semibold text-[28px] text-white">
                    {chatData.display_name || "Unknown Channel"}
                  </h1>
                  {chatData.username && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="rounded-[10px] p-4" style={{ background: "rgba(17,16,24,0.5)", border: "1px solid rgba(58,42,238,0.12)" }}>
                  <p className="font-sans text-[10px] text-white/40 uppercase tracking-wider mb-1">Subscribers</p>
                ...
                <div className="rounded-[10px] p-4" style={{ background: "rgba(17,16,24,0.5)", border: "1px solid rgba(58,42,238,0.12)" }}>
                  ...
                  <p className="font-sans text-[14px] text-white/70 capitalize">{chatData.chat_type || "channel"}</p>
                </div>
                <div className="rounded-[10px] p-4" style={{ background: "rgba(17,16,24,0.5)", border: "1px solid rgba(58,42,238,0.12)" }}>
