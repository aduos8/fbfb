...
type FormFieldCtx<TFieldValues extends FieldValues = FieldValues, TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>> = { name: TName };

const FormFieldCtx = React.createContext<FormFieldCtx>({} as FormFieldCtx);
...
const FormField = <TFieldValues extends FieldValues = FieldValues, TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>>({ ...props }: ControllerProps<TFieldValues, TName>) => (
  <FormFieldCtx.Provider value={{ name: props.name }}><Controller {...props} /></FormFieldCtx.Provider>
);

const useFormField = () => {
  const fieldCtx = React.useContext(FormFieldCtx);
  const itemCtx = React.useContext(FormItemCtx);
  ...
  const { id } = itemCtx;
  return { id, name: fieldCtx.name, formItemId: `${id}-form-item`, formDescriptionId: `${id}-form-item-description`, formMessageId: `${id}-form-item-message`, ...fieldState };
};

type FormItemCtx = { id: string };

const FormItemCtx = React.createContext<FormItemCtx>({} as FormItemCtx);

const FormItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  const id = React.useId();
  return <FormItemCtx.Provider value={{ id }}><div ref={ref} className={cn("space-y-2", className)} {...props} /></FormItemCtx.Provider>;
});
